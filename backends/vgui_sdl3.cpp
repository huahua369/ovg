...
bool widget_on_move(widget_t* wp, dev_event_t* dv, const glm::vec2& pos) {
	bool hover = false;
	if (!wp)return hover;
	auto e = &dv->v;
	auto t = dv->type;
	if (t == dev_event_type_e::mouse_move_e)
	{
		...
		if (k.x) {
			...
			{
				// 鼠标进入
				wp->on_mevent(event_type_e::on_enter, mps, p);
				if (wp->mevent_cb) {
					wp->mevent_cb(wp, event_type_e::on_enter, mps);
				}
			}
		}
		else {
			if (wp->_bst & (int)BTN_STATE::STATE_HOVER)
			{
				...
				// 鼠标离开
				wp->on_mevent(event_type_e::on_leave, mps, p);
				if (wp->mevent_cb) {
					wp->mevent_cb(wp, event_type_e::on_leave, mps);
				}
			}
		}

		{
			if (wp->_bst & (int)BTN_STATE::STATE_HOVER)
			{
				wp->on_mevent(event_type_e::on_move, mps, p);
				if (wp->mevent_cb)
				{
					wp->mevent_cb(wp, event_type_e::on_move, mps);
				}
			}
			if (wp->_bst & (int)BTN_STATE::STATE_ACTIVE) {
				...
				wp->has_drag = true;
				if (first) {
					wp->on_mevent(event_type_e::on_dragstart, dps, p);
					if (wp->mevent_cb)
					{
						wp->mevent_cb(wp, event_type_e::on_dragstart, dps);
					}
				}
				else
				{
					wp->on_mevent(event_type_e::on_drag, dps, p);		// 拖动事件
					if (wp->mevent_cb)
					{
						wp->mevent_cb(wp, event_type_e::on_drag, dps);		// 拖动事件
					}
				}
			}
		}
...
void widget_on_event(widget_t* wp, dev_event_t* dv, const glm::vec2& pos) {
	if (!wp)return;
	auto e = &dv->v;
	auto t = dv->type;
	//wp->form = ep->form;
	...
	case dev_event_type_e::mouse_button_e:
	{
		...
		if (!cs && wp->_bst & (int)BTN_STATE::STATE_HOVER) {
			...
			if (p->button == 1) {
				if (p->down == 1) {
					...
					//wp->cks = 0;
					wp->on_mevent(event_type_e::on_down, mps, p);
					if (wp->mevent_cb) { wp->mevent_cb(wp, event_type_e::on_down, mps); }
				}
				else {
					if (wp->_bst & (int)BTN_STATE::STATE_ACTIVE)
					{
						...
						{
							wp->on_mevent(event_type_e::on_up, mps, p);
							if (wp->mevent_cb) {
								wp->mevent_cb(wp, event_type_e::on_up, mps);
							}
		...
		if (p->down == 0) {
			...
			if (wp->has_drag)
			{
				wp->on_mevent(event_type_e::on_dragend, mps, p);
				if (wp->mevent_cb) {
					wp->mevent_cb(wp, event_type_e::on_dragend, mps);
				}
			}
			wp->has_drag = false;
			wp->on_mevent(event_type_e::mouse_up, mps, p);
			if (wp->mevent_cb) {
				wp->mevent_cb(wp, event_type_e::mouse_up, mps);
			}
		}
	}
	break;
	case dev_event_type_e::mouse_wheel_e:
	{
		...
		{
			dv->ret = wp->on_mevent(event_type_e::on_scroll, mps, p);
			if (wp->mevent_cb) {
				wp->mevent_cb(wp, event_type_e::on_scroll, mps);
			}
			dv->ret = 1;
		}
	}
	break;
	case dev_event_type_e::text_input_e:
	{
		auto p = e->t;
		auto ipos = wp->input_pos();// 计算输入法坐标
		p->x = ipos.x;
		p->y = ipos.y + 3;
		p->w = ipos.z;
		p->h = ipos.w;
		wp->add_text(p->text, strlen(p->text));
	}break;
	case dev_event_type_e::text_editing_e: {
		if (!wp->is_input)break;
		auto& p = e->e;
		bool setimepos = false;
		if (wp->cursor.x != wp->cursor.y)
		{
		...
		wp->set_editing(p->text, p->length, p->start);
	}break;
	case dev_event_type_e::keyboard_e:
	{
		if (wp->on_event_cb)
			wp->on_event_cb(dv, pos);
	}break;
	default:
		break;
	}
...
bool on_gui_event(widget_t* pw, dev_event_t* dv, const glm::ivec2& ppos)
{
	...
	if (pw->on_event_cb)
	{
		pw->on_event_cb(dv, ppos);
	}
	else
	{
		if (dv->ret && t == dev_event_type_e::mouse_button_e)
		{
			auto p = e->b;
			if (p->down == 1 /*&& pw->_bst & event_type_e::on_down*/)
			{
